...
4. Pour obtenir un token de test, activez temporairement **Direct access grants** dans la config du client `cook-me-mobile` (Keycloak Admin → Clients → cook-me-mobile → Settings → Capability config), puis utilisez curl :
...
> ⚠️ **Note** : Désactivez **Direct access grants** une fois le test terminé (cette option est uniquement nécessaire pour `grant_type=password`).
